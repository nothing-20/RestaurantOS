...
/**
 * Canonical generator for staff activation URLs.
 * Every invitation link across all owner/staff flows MUST be generated by this function.
 */
export function buildStaffActivationLink(token: string, email: string, employeeId: string): string {
  const origin =
    typeof window !== 'undefined'
      ? window.location.origin
      : 'https://restaurant-os-dun.vercel.app';
...
export const invitationService = {
  ...
  async createStaffInvitation(params: CreateInvitationParams): Promise<CreateInvitationResult> {
    ...
    // 2. Generate secure client-side activation token & expiration (7 days)
    const secureToken = generateSecureToken();
    const now = new Date().toISOString();
    const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();

    // 3. Create document in root-level employees collection
    ...
    // 4. Construct canonical Activation Link using the exact created Firestore employee document ID
    const activationLink = buildStaffActivationLink(secureToken, trimmedEmail, employeeDoc.id);

    console.info('[StaffInvitation] Activation link generated', {
      employeeId: employeeDoc.id,
      hasToken: Boolean(secureToken),
      email: trimmedEmail,
    });
