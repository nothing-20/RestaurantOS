...
export const OwnerBilling: React.FC = () => {
  ...
  // Configurable values (with tenant setting fallbacks)
  const taxRate = tenantSettings?.taxPercent ?? 5; // e.g. 5% GST
  const serviceChargeRate = tenantSettings?.serviceCharge ?? 5; // e.g. 5% service charge
  const currencySymbol = tenantSettings?.currency === 'INR' ? '₹' : tenantSettings?.currency === 'USD' ? '$' : '₹';

  // Role gate security helper
  ...
  useEffect(() => {
    ...
    // 2. Subscribe to Tables (for checkout queue status)
    ...
    // 4. Subscribe to Transactions (recent log)
  ...
  const shiftCalculations = useMemo(() => {
    ...
    shiftTransactions.forEach(t => {
      ...
      // Add complimentary items values from transaction snapshot if stored
      const items = t.items || [];
  ...
  // Helper stats computed live for the overview cards
  const stats = useMemo(() => {
  ...
  // Tab definitions (v1.1 with Shift Reports added)
  const tabsList = [
  ...
  // Open Bill Calculations
  const orderSubtotal = useMemo(() => {
    if (!selectedOrder) return 0;
    return selectedOrder.items.reduce((sum, item) => {
      // v1.1 Complimentary Items count as 0 for calculations
  ...
  // v1.1 Cash Drawer Shift Handlers
  ...
  const handleResumeBill = async (order: IOrder) => {
    ...
    try {
      ...
      // Open Checkout drawer for resumed bill
  ...
  // v1.1 Complimentary Items Handlers
  ...
  // Supervisor PIN checks for large discounts
  ...
  // Discount Selection change handler
  ...
  // Check quantities modification with dialog confirmation
  ...
  // Finalize Checkout and Payment complete
  const handleCompletePayment = async () => {
    ...
    // Check payment modes validation
    ...
    try {
      ...
      // Update Table Status to dirty/cleaning
  ...
  return (
    <div className="space-y-6 text-left select-none relative">
      ...
      {/* Main Page title & Live stats bar */}
      ...
      {isLoading ? (
        ...
        <div className="relative z-10">
          ...
          {/* TAB 1: BILLING QUEUE & HELD BILLS */}
          {activeTab === 'billing_queue' && (
            <div className="space-y-6">
              
              {/* Billing requests section */}
                        ...
                        <Card key={order.orderId} className="p-5 border-amber-500/20 bg-amber-950/5 hover:border-amber-500/40 transition-all flex flex-col justify-between h-48">
                          ...
                          <div className="mt-4 flex gap-2">
                            <Button 
                              ...
                              className="flex-1 text-xs font-semibold py-2 border-slate-850 bg-slate-950 text-slate-300"
                            >
                              <Eye className="w-3.5 h-3.5 mr-1" />
                              View Items
                            </Button>
              ...
              {filteredPaidBills.length === 0 ? (
                ...
                <Card className="p-0 overflow-hidden border-slate-850">
                  <table className="w-full text-left text-xs text-slate-300">
                    <thead className="bg-slate-950/40 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-850">
                      <tr>
                        ...
                        <tr key={order.orderId} className="hover:bg-slate-900/20">
                          ...
                          <td className="p-3 text-slate-450">
                            {order.reprintCount ? (
                              ...
                              <span className="text-[10px] text-slate-550">Original Only</span>
                            )}
                          </td>
          ...
          {activeTab === 'refunds' && (
            <div className="space-y-6">
              ...
              {/* Renders Paid Bills lists ready for refunds */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                
                {/* Left: Settled invoices list */}
                <div className="lg:col-span-2 space-y-4">
                  <h3 className="text-xs font-bold text-slate-450 uppercase tracking-wider">Select Transaction for Refund</h3>
                        ...
                        {paidBillsOrders.map((order) => (
                          ...
                          >
                            <div>
                              ...
                              <p className="text-[10px] text-slate-500 mt-1">Processed: {formatTimestamp(order.paidAt || order.createdAt)} · Waiter: {order.waiterName}</p>
                            </div>
                ...
                {/* Right: Refund History logger list */}
                <div className="space-y-4">
                  ...
                  {refunds.length === 0 ? (
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                      {refunds.map((ref) => (
                        <Card key={ref.id} className="p-3 border-slate-850 bg-slate-950/20 text-[11px] space-y-2">
                          <div className="flex justify-between items-center">
                          ...
                          <p className="text-[10px] text-slate-450 italic bg-slate-900/40 p-2 rounded border border-slate-850">Reason: {ref.reason}</p>
          ...
          {activeTab === 'sales_summary' && (
            <div className="space-y-6">
              ...
              {/* Sales Report Cards grid */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                
                {/* Payment mode distributions card */}
                <Card className="p-5 border-slate-850 bg-slate-900/40 space-y-4">
                  <div>
                    <h3 className="font-display font-bold text-xs text-slate-450 uppercase tracking-widest">Payments Distribution</h3>
                ...
                {/* Peak Billing Hours report panel */}
                <Card className="p-5 border-slate-855 bg-slate-900/40 space-y-4">
                  <div>
                    <h3 className="font-display font-bold text-xs text-slate-450 uppercase tracking-widest">Peak Billing Times</h3>
      ...
      {/* DRAWERS / MODALS MODULES */}

      {/* MODAL: SHIFTS DETAILED REPORT OVERLAY */}
      <Modal
              ...
              <div className="space-y-1.5 border-b border-slate-850 pb-3 font-bold">
                ...
                {selectedShiftReport.difference !== undefined && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={selectedShiftReport.difference === 0 ? 'text-emerald-450' : selectedShiftReport.difference < 0 ? 'text-red-400' : 'text-amber-400'}>
      ...
      {/* MODAL 1: VIEW ORDER DETAILS ITEM CHECKLIST */}
      <Modal
      ...
      >
        {selectedOrder && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-left text-xs max-h-[80vh] overflow-y-auto">
            ...
            {/* Left Column: Items Review & Modification */}
            <div className="space-y-4">
              <div className="flex justify-between items-center">
              ...
              <div className="border border-slate-850 bg-slate-950/20 rounded-xl divide-y divide-slate-850 max-h-64 overflow-y-auto">
                {selectedOrder.items.map((item) => (
                  <div key={item.itemId} className="p-3 flex justify-between items-center gap-3">
                    ...
                    {/* v1.1 Complimentary Toggle Star button (Owner only) */}
                    {isAuthorizedOwner && (
                      <Button
                        variant="ghost"
                        size="sm"
              ...
              {/* Future addition UI Placeholder */}
            ...
            {/* Right Column: Calculations & Payment settles */}
            <div className="space-y-4 border-l border-slate-850 pl-0 lg:pl-6">
              
              {/* Discounts Selection Segment */}
                ...
                {paymentMode === 'mixed' && (
                  <Card className="p-3 border-slate-850 bg-slate-900/35 space-y-3">
                    ...
                    <div className="flex justify-between items-center text-[10px] pt-1.5 border-t border-slate-850">
                      ...
                      <span className={Math.abs(totalPaidInMixed - roundedTotal) <= 2 ? 'text-emerald-450 font-bold' : 'text-red-400 font-bold'}>
              ...
              {/* v1.1 Checkout / Hold Bill controls */}
      ...
      {/* MODAL 3: INVOICE PREVIEW SCREEN (PROFESSIONAL BILL PRINT-OUT) */}
      <Modal
        ...
        {selectedOrder && (
          <div className="space-y-6 text-left text-[11px] text-slate-355">
            ...
            <div id="invoice-sheet" className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 font-mono select-text relative overflow-hidden">
              ...
              {/* v1.1 Reprint Watermark indicators */}
              ...
              {/* Settlements Computations */}
              <div className="space-y-1 border-b border-dashed border-slate-800 pb-3 text-[10px] text-slate-450">
                <div className="flex justify-between">
                  <span>Subtotal:</span>
                ...
                {selectedOrder.roundOff ? (
                  <div className="flex justify-between text-[9px] text-slate-555">
                    <span>Round Off:</span>
              ...
              {/* UPI/Mixed Payment Settled Details */}
              ...
              {/* UPI Scan QR Placeholder */}
              <div className="flex flex-col items-center justify-center p-3 border border-slate-800 rounded-xl bg-slate-955/40 text-center gap-1.5">
            ...
            {/* Sharing / Printing / v1.1 Reprint triggers */}
      ...
      {/* MODAL 4: SUPERVISOR/OWNER PASSWORD VALIDATION FOR DISCOUNTS */}
      <Modal
      ...
      >
        {refundTargetOrder && (
          <div className="space-y-4 text-left text-xs">
            <div className="p-3 bg-slate-955/40 border border-slate-850 rounded-xl space-y-1 font-medium">
              <div className="flex justify-between">
        ...
        {activeShift && (
          <div className="space-y-4 text-left text-xs font-mono max-h-[85vh] overflow-y-auto pr-1">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button 
                variant="primary" 
                className="flex-1 font-bold bg-red-500 hover:bg-red-650 text-white" 
