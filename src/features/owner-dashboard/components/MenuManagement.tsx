  ...
  const onSubmitItem = async (data: TMenuItemForm) => {
    ...
    try {
      ...
      const itemData: Omit<IMenuItem, 'id'> = {
        ...
        category: catName, // backward compatibility
        ...
        isVeg: data.isVeg,
        veg: data.isVeg, // backward compatibility
        isAvailable: data.isAvailable,
        available: data.isAvailable, // backward compatibility
  ...
  const toggleItemAvailable = async (item: IMenuItem) => {
    ...
    try {
      ...
      await updateDoc(docRef, { 
        isAvailable: !item.isAvailable,
        available: !item.isAvailable // backward compatibility
      });
      ...
      <div className="flex border-b border-slate-800/60 pb-px gap-2 flex-wrap">
        {[
          ...
          return (
            ...
            >
              {tab.id === 'availability' ? (
                // Use a simplified toggle icon since Switch component conflict
      ...
      >
        <form onSubmit={handleSubmitCategory(onSubmitCategory)} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <span className="text-xs font-semibold text-slate-400 select-none">Or Upload Photo</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col space-y-1.5 justify-end pb-1.5">
              <span className="text-xs font-semibold text-slate-400 select-none">Status</span>
              <Switch 
                checked={true}
                onChange={() => {}}
                label="Is Active"
