...
export const OwnerTablesManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Subscribe to Seating Layout document
    ...
    // Subscribe to Seating Tables collection
  ...
  const onSubmitForm = async (data: TTableForm) => {
    ...
    // Check duplicate table code/number or name
    ...
    try {
      ...
      const tableData = {
        ...
        number: data.tableNumber.trim(), // backward compatibility
        floor: data.floor,
        section: data.section,
        capacity: data.capacity,
        seatingCapacity: data.capacity, // backward compatibility
        status: data.status,
        tableStatus: data.status, // backward compatibility
        shape: data.shape,
  ...
  const toggleTableActive = async (tbl: ITableItem) => {
    if (!user?.tenantId) return;
    try {
      const docRef = doc(db, 'restaurants', user.tenantId, 'tables', tbl.id);
      await updateDoc(docRef, { 
        isActive: !tbl.isActive,
        status: !tbl.isActive ? 'Available' : 'Disabled'
      });
      toast.success(`${tbl.tableName} status updated.`);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const getStatusColor = (status: ITableItem['status']) => {
    switch (status) {
      ...
      default: return 'info'; // Ordering, Preparing, Dining, Bill Requested
    }
  };
  ...
  const getStatusColorHex = (status: ITableItem['status']) => {
    switch (status) {
      ...
      default: return 'border-cyan-500/50 bg-cyan-950/20 text-cyan-400 shadow-cyan-500/5'; // Ordering, Dining, etc
    }
  };
  ...
  const handleDownloadQr = async (tbl: ITableItem) => {
    ...
    try {
      ...
      // Draw details Card on canvas for download
      ...
      >
        {qrTable && (
          <div className="space-y-4 text-center">
            {/* High-fidelity generated code placeholder */}
