...
export const OwnerStaffManager: React.FC = () => {
  ...
  const fetchEmployees = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      // If index not built yet, fall back to unordered query
      try {
        const empRef = collection(db, 'employees');
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Also update users/{uid} if activated
  ...
  const handleResendInvite = async (emp: IEmployee) => {
    // In production you would trigger an email here via Cloud Functions.
    // For now copy the activation link to clipboard and show a toast.
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // If activated, also remove the users/{uid} record
      if (emp?.firebaseUid) {
        try {
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleEditSubmit} className="space-y-4 text-left">
          {renderFormFields(true /* disable email on edit */)}
          <div className="flex items-center space-x-3 pt-4 border-t border-slate-800/40">
