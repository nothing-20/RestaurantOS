    ...
    <div className="space-y-6 text-left select-none pb-24 font-sans">
      ...
      <div className="bg-white border border-[#E3DED5] rounded-xl p-6 shadow-[0_1px_4px_rgba(30,30,20,0.05)]">
        ...
        <p className="text-xs text-[#6F746F] mt-1 font-normal">
          Toggle dishes in or out of stock in real-time. Customer menus and waiter apps reflect adjustments instantly.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="h-64 flex flex-col items-center justify-center text-center border border-dashed border-[#E3DED5] rounded-xl bg-white">
          <AlertTriangle className="w-8 h-8 text-[#6F746F]/50 mb-2" />
          <p className="text-sm font-semibold text-[#6F746F]">No items match your criteria.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredItems.map((item) => (
            <div 
              key={item.id} 
              className={`p-4 border border-[#E3DED5] bg-white rounded-xl shadow-[0_1px_4px_rgba(30,30,20,0.05)] flex items-center justify-between space-x-4 transition-all duration-200 ${
                !item.available ? 'border-[#C7463A]/40 bg-[#FDEEEC]/30' : ''
              }`}
            >
              <div className="flex-1 space-y-1">
                <div className="flex items-center space-x-2">
                  <h3 className="font-serif font-bold text-base text-[#18201D]">{item.name}</h3>
                  {item.veg ? (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded border border-[#287A55]/30 bg-[#EBF7EE] text-[#287A55]">Veg</span>
                  ) : (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded border border-[#C7463A]/30 bg-[#FDEEEC] text-[#C7463A]">Non-Veg</span>
                  )}
                </div>
                <div className="text-xs font-bold font-mono text-[#18201D]">{formatPrice(item.price)}</div>
                <span className="inline-block text-[10px] font-semibold text-[#6F746F] bg-[#F7F4EE] border border-[#E3DED5] px-2 py-0.5 rounded">
                  {item.category}
                </span>
              </div>
              ...
              <div className="shrink-0 flex flex-col items-end space-y-1.5">
                <span className={`text-[10px] font-bold uppercase tracking-wider ${item.available ? 'text-[#287A55]' : 'text-[#C7463A]'}`}>
                  {item.available ? 'In Stock' : 'Out of Stock'}
                </span>
                <Switch 
                  checked={item.available} 
                  onChange={() => handleToggleAvailable(item)} 
                  className="accent-[#13241F]"
                />
              </div>
            </div>
          ))}
